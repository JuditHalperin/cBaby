...
class SecondPage:
    ...
    def __init__(self, vs):
        ...
        # thread to hear the 'bye' command in the background
        self.listen = True
        self.query = None
    ...
    def moving(self, frame1, frame2):
        """The function gets two frames, and checks if there is a significant moving"""
        ...
        if self.TWO_MINS > 0:  # if we started to count 2 mins
            if self.TWO_MINS == 120:  # after 2 mins
                if self.DELTA >= 120 * 20:  # if there was a significant moving - send a message
                    ...
                    self.TWO_MINS = 0  # else- we didn't find significant of moving, restart the counters
                    self.DELTA = 0
            elif sum > 20:  # if sum>20 - add 1 to 2 mins and add sum to delta
                self.TWO_MINS += 1
                self.DELTA += sum
            else:  # else - we didn't find a significant moving, restart the counters
                self.TWO_MINS = 0
                self.DELTA = 0
        elif sum > 20:  # if we didn't start to count 2 mins, and sum>20 - start count 2 mins and add sum to delta
            self.TWO_MINS += 1
            self.DELTA += sum

    def face_in_image(self, frame):
        """Get a frame, convert to grayscale and find face in the frame"""
    ...
    def face_recognition(self, frame):
        ...
        if self.HALF_MIN > 0:
            ...
            # if we didn't start to count and there is no face in the img
            self.HALF_MIN += 1
    ...
    def color_recognition(self, frame, first_color):
        """Get a frame and the color of the face in the first frame, and check if there is a significant different in the face color"""
    ...
    def __show_alert(self, text):
        """This function shows the alert message"""
...
def start():
    ...
    vs = VideoStream(src=0).start()  # start the video stream thread, 0 for webcam
    time.sleep(1)  # pause for a second to allow the camera sensor to warm up

    sp = SecondPage(vs)  # start the loop
